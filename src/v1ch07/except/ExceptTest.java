...
class ExceptTestPanel extends Box
{
   ...
   public ExceptTestPanel()
   {
      ...
      addRadioButton("Integer divide by zero", event -> a[1] = 1 / (a.length - a.length));

      addRadioButton("Floating point divide by zero", event ->  a[1] = a[2] / (a[3] - a[3]));

      addRadioButton("Array bounds", event -> a[1] = a[10]);

      addRadioButton("Bad cast", event -> a = (double[]) event.getSource());

      addRadioButton("Null pointer", event -> System.out.println(textField.getAction().toString()));

      addRadioButton("sqrt(-1)", event -> a[1] = Math.sqrt(-1));

      addRadioButton("Overflow", event -> {
         a[1] = 1000 * 1000 * 1000 * 1000;
         int n = (int) a[1];
         System.out.println(n);
      });

      addRadioButton("No such file", event -> {
         try
         {
            System.out.println(
      ...
      addRadioButton("Throw unknown", event -> {
